"""services/remember.py: the "remember this device" grant behind the `mw_remember`
...
async def test_revoke_removes_the_named_device_only(db, make_member):
...
async def test_revoke_all_for_member_clears_every_device(db, make_member):
...
async def test_purge_expired_removes_only_past_expiry_rows(db, make_member):
    ...
    rows = (await db.execute(select(RememberedDevice))).scalars().all()
    ...
    remaining = (await db.execute(select(RememberedDevice))).scalars().all()
