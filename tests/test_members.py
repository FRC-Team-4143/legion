...
async def test_serialize_member_shape(db, make_member):
    ...
    await make_member(
        ...
        focus_slug="design", slack="U0LEAD", is_lead=True,
    )
    m = (
        await db.execute(
            select(Member)
    ...
    assert data["focus_group"] == {"slug": "design", "label": "Design"}
...
async def test_member_without_team_or_focus_serializes_null(db, make_member):
    ...
    await make_member(name="No Team", team_number=None, focus_slug=None)
    m = (
        await db.execute(
            select(Member)
            .options(selectinload(Member.team), selectinload(Member.focus_group))
    ...
    assert data["focus_group"] is None
