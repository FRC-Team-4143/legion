...
engine opens any connection (see app/database.py). Mirrors the sibling apps.
"""
import logging
import os
import sqlite3
...
def sqlite_path() -> Optional[str]:
    ...
    if not url.startswith("sqlite"):
        return None
    # e.g. "sqlite+aiosqlite:///./legion.db" -> "./legion.db"
...
def stage_restore(upload_bytes: bytes) -> tuple[bool, str]:
    ...
    if not validate_sqlite_file(tmp):
        os.remove(tmp)
        return False, "That file is not a valid Legion database backup."
...
def nightly_backup() -> Optional[str]:
    ...
    if not is_sqlite():
        return None
    name = f"legion-{datetime.now():%Y%m%d-%H%M%S-%f}.db"
