...
async def require_api_key(x_api_key: str = Header(default="")) -> None:
    ...
    consumer (Tempus, Munus, Virtus, Merces) has its own key — any is accepted here, so a
    leak or rotation of one never affects the others."""
    keys = [
        k for k in (
            settings.tempus_api_key, settings.munus_api_key,
            settings.virtus_api_key, settings.merces_api_key,
        )
        if k
    ]
    if not keys:
