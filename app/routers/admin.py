...
from app.models import (
    AuditLog, FocusGroup, Member, MemberRole, Team, role_label,
)
...
async def _active_focus_groups(db: AsyncSession):
    return (
        await db.execute(
            select(FocusGroup).where(FocusGroup.is_active.is_(True))
            .order_by(FocusGroup.sort_order, FocusGroup.label)
        )
    ).scalars().all()
...
async def admin_dashboard(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    stats = {
        ...
        "focus_groups": await db.scalar(
            select(func.count()).select_from(FocusGroup).where(FocusGroup.is_active.is_(True))
        ) or 0,
    }
    return templates.TemplateResponse(
...
async def admin_members_list(
    ...
    return templates.TemplateResponse(
        "admin/members.html",
        {
            ...
            "focus_groups": await _active_focus_groups(db),
...
async def admin_members_create(
    ...
    focus_group_id: Optional[str] = Form(None),
    ...
    member = Member(
        ...
        focus_group_id=_opt_id(focus_group_id),
...
async def admin_members_edit_post(
    ...
    # member_code is Legion's canonical identity and is intentionally never recomputed.
    member.name = name.strip()
    member.role = MemberRole(role)
    member.team_id = _opt_id(team_id)
    member.focus_group_id = _opt_id(focus_group_id)
...
async def admin_focus_groups_list(request: Request, db: AsyncSession = Depends(get_db)):
    if redirect := _require_auth(request):
        return redirect
    groups = (
        await db.execute(
            select(FocusGroup, func.count(Member.id))
            .outerjoin(Member, Member.focus_group_id == FocusGroup.id)
            .group_by(FocusGroup.id)
            .order_by(FocusGroup.sort_order, FocusGroup.label)
        )
    ).all()
    return templates.TemplateResponse(
        "admin/focus_groups.html",
        {
            "request": request,
...
@router.post("/focus-groups")
async def admin_focus_groups_create(
    request: Request,
    label: str = Form(...),
    ...
    if not the_slug:
        return RedirectResponse("/admin/focus-groups?error=Invalid+slug", status_code=303)
    existing = (await db.execute(select(FocusGroup).where(FocusGroup.slug == the_slug))).scalars().first()
    if existing:
        return RedirectResponse(f"/admin/focus-groups?error=Slug+{the_slug}+already+exists", status_code=303)
    max_order = await db.scalar(select(func.max(FocusGroup.sort_order))) or 0
    db.add(FocusGroup(slug=the_slug, label=label.strip(), sort_order=max_order + 1))
    await audit.record(db, request, "focus_group.create", f"Created focus group {label.strip()}", entity_type="focus_group")
    await db.commit()
    return RedirectResponse("/admin/focus-groups", status_code=303)


@router.post("/focus-groups/{group_id}/edit")
async def admin_focus_groups_edit(
    group_id: int,
    request: Request,
    label: str = Form(...),
    db: AsyncSession = Depends(get_db),
    ...
    if redirect := _require_auth(request):
        return redirect
    group = (await db.execute(select(FocusGroup).where(FocusGroup.id == group_id))).scalars().first()
    if group:
        group.label = label.strip()
        await audit.record(db, request, "focus_group.edit", f"Renamed focus group to {group.label}", entity_type="focus_group", entity_id=group.id)
        await db.commit()
    return RedirectResponse("/admin/focus-groups", status_code=303)


@router.post("/focus-groups/{group_id}/toggle")
async def admin_focus_groups_toggle(group_id: int, request: Request, db: AsyncSession = Depends(get_db)):
    """Retire / reactivate a focus group without deleting it (keeps historical assignments)."""
    if redirect := _require_auth(request):
        return redirect
    group = (await db.execute(select(FocusGroup).where(FocusGroup.id == group_id))).scalars().first()
    if group:
        ...
        await audit.record(db, request, "focus_group.toggle", f"{state.capitalize()} focus group {group.label}", entity_type="focus_group", entity_id=group.id)
        await db.commit()
    return RedirectResponse("/admin/focus-groups", status_code=303)
...
async def admin_import_post(request: Request, file: UploadFile = File(...), db: AsyncSession = Depends(get_db)):
    ...
    focus = {g.slug: g for g in (await db.execute(select(FocusGroup))).scalars().all()}
    ...
    for i, row in enumerate(reader, start=2):  # row 1 = header
        ...
        focus_str = (row.get("focus_group") or "").strip().lower()
        ...
        fg = None
        if focus_str:
            if focus_str not in focus:
                errors.append({"row": i, "reason": f"Unknown focus group '{focus_str}'", "data": dict(row)})
                continue
            fg = focus[focus_str]
        ...
        if existing:
            ...
            existing.focus_group_id = fg.id if fg else None
            if slack_uid:
            ...
            db.add(Member(
                ...
                focus_group_id=fg.id if fg else None,
                slack_user_id=slack_uid,
