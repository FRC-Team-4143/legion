...
async def admin_members_list(
    request: Request, status: str = "active", db: AsyncSession = Depends(get_db)
):
    """Sorting/column-filtering on this page is client-side (Excel-style, see
    static/js/table-filter-sort.js), but the active/archived/all mode is still applied
    at the query level — keeps archived members out of the DOM entirely (and therefore
    out of every column's filter dropdown) unless the toggle above the table is set to
    Archived or All. Any value other than "active"/"archived"/"all" falls back to
    "active"."""
    if redirect := _require_staff(request):
        return redirect
    ...
    if status == "archived":
        q = q.where(Member.is_active.is_(False))
    elif status != "all":
        status = "active"
